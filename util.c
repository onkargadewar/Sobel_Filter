...
void error(cl_int err, char* actionName, ...)
{
	...
	va_list args1, args2;
	va_start(args1, actionName);
	va_copy(args2, args1);
	char* tmp = malloc((vsnprintf(NULL, 0, actionName, args1) + 1) * sizeof(char));
	vsprintf(tmp, actionName, args2);
	printf("Error while %s. (error ID %d, %s)\n", tmp, err, get_error_string(err));
	va_end(args1);
	va_end(args2);
	free(tmp);
	exit(-1);
